...
def get_group_records(
    pool: ydb.QuerySessionPool, group: Group, is_exist_check: bool
) -> list[ydb.convert._Row]:
    attr = "id"
    if is_exist_check:
        attr = "1"
...
def select_id_group(pool: ydb.QuerySessionPool, group: Group) -> int:
    records = sorted(
        get_group_records(pool, group, False), key=lambda x: x.id, reverse=True
    )
    return records[0].id
...
def get_lesson_records(
    ...
    lesson: Union[GroupLesson, PersonalLesson],
    id_obj: int,
    is_exist_check: bool,
) -> list[ydb.convert._Row]:
    attr = "id"
...
def select_id_lesson(
    pool: ydb.QuerySessionPool, lesson: Union[GroupLesson, PersonalLesson], id_obj: int
) -> int:
    records = sorted(
        get_lesson_records(pool, lesson, id_obj, False),
        key=lambda x: x.id,
        reverse=True,
    )
    return records[0].id
...
def get_student_records(
    pool: ydb.QuerySessionPool, student: Student, is_exist_check: bool
) -> list[ydb.convert._Row]:
    attr = "id"
    if is_exist_check:
        attr = "1"
...
def select_id_student(pool: ydb.QuerySessionPool, student: Student) -> int:
    records = sorted(
        get_student_records(pool, student, False), key=lambda x: x.id, reverse=True
    )
    return records[0].id
...
def get_lecturer_records(
    pool: ydb.QuerySessionPool, lecturer: Lecturer, is_exist_check: bool
) -> list[ydb.convert._Row]:
    attr = "id"
    if is_exist_check:
        attr = "1"
...
def select_id_lecturer(pool: ydb.QuerySessionPool, lecturer: Lecturer) -> int:
    records = sorted(
        get_lecturer_records(pool, lecturer, False), key=lambda x: x.id, reverse=True
    )
    return records[0].id
...
def connect_lecturer_with_group(
    pool: ydb.QuerySessionPool, id_group: int, id_lecturer: int
):
