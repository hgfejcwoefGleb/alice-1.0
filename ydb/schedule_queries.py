...
def find_lesson_student(
    pool: QuerySessionPool,
    is_group_lesson: bool,
    ...
    dtype = (
        "Utf8"
        if search_attr_name == "name"
        else "Int64" if search_attr_name == "id_lecturer" else "Date"
    )
    if is_group_lesson:
        return pool.execute_with_retries(
            ...
            """,
            {
                f"${search_attr_name}": (
                    search_attr_val,
                    (
                        ydb.PrimitiveType.Date
                        if search_attr_name == "lesson_date"
                        else (
                            ydb.PrimitiveType.Int64
                            if search_attr_name == "id_lecturer"
...
def find_by_week_day_lesson_student(
    ...
    if is_group_lesson:
        ...
        return pool.execute_with_retries(
            f"""
                    ...
                    """,
            {f"$lesson_date": week_days_dict[week_day], "$id_student": id_student},
        )[0].rows


def find_lesson_lecturer(
    pool: QuerySessionPool,
    table_name: str,
    search_attr_name: str,
    search_attr_val: str,
...
def find_by_week_day_lesson_lecturer(
    pool: ydb.QuerySessionPool, table_name: str, week_day: str, id_lecturer: int
) -> list:
    return pool.execute_with_retries(
        f"""
                        DECLARE $lesson_date AS Int64;
...
def insert_lesson_data(
    pool: ydb.QuerySessionPool, lesson: Union[GroupLesson, PersonalLesson], id_elem: int
):
    unique_elem = ""
    table = ""
    ...
    pool.execute_with_retries(
        ...
        """,
        {
            ...
            "$lesson_date": (
                datetime.strptime(lesson.lesson_date, "%d.%m.%Y").date(),
                ydb.PrimitiveType.Date,
            ),
...
def insert_help_tables_data(
    pool: ydb.QuerySessionPool, id_lesson: int, id_obj: int, is_group_lesson: bool
):
    table, field_1, field_2 = "GroupLessonGroup", "id_group_lesson", "id_group"
    if not is_group_lesson:
        table, field_1, field_2 = (
            "PersonalLessonStudent",
            "id_personal_lesson",
            "id_student",
        )
    pool.execute_with_retries(
    ...
    if is_student:
        ...
        if is_group_lesson:
            ...
            if not is_lesson_reg(pool, lesson, student.id_group):
                ...
                insert_help_tables_data(
                    pool, id_group_lesson, student.id_group, is_group_lesson
                )
        elif not is_group_lesson:
            ...
            insert_help_tables_data(
                pool, id_personal_lesson, id_student, is_group_lesson
            )
    else:
        ...
        if is_group_lesson:
            ...
            if not is_lesson_reg(pool, lesson, lesson.id_group):
                ...
                id_group_lesson = select_id_lesson(pool, lesson, lesson.id_group)
                insert_help_tables_data(
                    pool, id_group_lesson, lesson.id_group, is_group_lesson
                )
        else:
            lesson = PersonalLesson(*lesson_data)
            id_lecturer = select_id_lecturer(pool, lecturer)
